...
* `name` - (Optional) The name of the load balancer.
    ...
    * _Constraints_:
        ...
        * The value cannot start with prefix `internal-`
...
-> **Note** If `name` and `name_prefix` are not specified, Terraform will autogenerate a name with a prefix `tf-lb`.
...
* `subnet_mapping` - (Optional) List of subnet ID to IP address mappings.
  The structure of this block is [described below](#subnet_mapping).
    * _Constraints_: `subnet_mapping` is required if `subnets` is not specified
* `subnets` - (Optional, Editable) List of subnet IDs.
    * _Constraints_:
        * `subnets` is required if `subnet_mapping` is not specified
        * All subnets must be from different available zones

~> **Note** You can only add new subnets to the `subnets` list, removing subnets is not available.
...
* `allocation_id` - (Optional) The ID of the Elastic IP address allocation.
  The **internet-facing** load balancer will be available at this IP address.
* `private_ipv4_address` - (Optional) The private IP address within the specified subnet.
  The **internal** load balancer will be available at this IP address.

~> **Note** All subnets specified in the `subnet_mapping` blocks must be from different available zones.
...
Load balancer can be imported using `arn`, e.g.,
