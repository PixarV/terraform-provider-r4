...
* `name` - (Optional) The name of the target group.
    * _Length of the value_: From 1 to 32 symbols
    * _Constraints_:
        * `name` cannot be specified with `name_prefix`
...
* `name_prefix` - (Optional) Creates a unique name beginning with the specified prefix.
    * _Length of the value_: From 1 to 6 symbols
    * _Constraints_:
        * `name_prefix` cannot be specified with `name`
        * The value constraints are the same as `name`

-> **Note** If `name` and `name_prefix` are not specified, Terraform will autogenerate a name with a prefix `tf-`.

* `protocol_version` - (Optional) The version of HTTP protocol.
    * _Valid values_: `HTTP1`, `HTTP2`
...
* `target_type` - (Optional) The type of the target. All targets in a target group must have the same type.
    * _Valid values_: `instance`
    * _Default value_: `instance`
...
* `enabled` - (Optional, Editable) Indicates whether health check will be enabled.
    ...
    * _Constraints_: Health check must be enabled for target groups with instance target type
* `healthy_threshold` - (Optional, Editable) Number of consecutive successful health checks after which the target status changes to healthy.
    * _Valid values_: From 2 to 10
    * _Constraints_: For TCP and UDP target groups `healthy_threshold` and `unhealthy_threshold` must be the same
* `interval` - (Optional, Editable) The amount of time, in seconds, between health checks on an individual target.
    * _Valid values_: From 5 to 300
    * _Default value_: 30
* `port` - (Optional, Editable) The port used to perform health checks on targets.
  If value is `traffic-port`, the port, on which the target receives requests, is used for health checks.
    * _Valid values_: `traffic-port`
    * _Default value_: `traffic-port`
* `protocol` - (Optional, Editable) The protocol used to perform health checks on targets.
    * _Valid values_: `TCP`, `UDP`, `HTTP`
    * _Constraints_: `health_check.protocol` and target group `protocol` must be the same
* `timeout` - (Optional, Editable) The amount of time, in seconds, during which no response means a failed health check.
    * _Valid values_: From 2 to 120
    * _Constraints_: You cannot set a custom timout value for TCP and UDP target groups (default is 10).
...
Target group can be imported using `arn`, e.g.,
