...
* `priority` - (Optional) Priority of the rule.
  If `priority` is not set, it will receive the value of next highest available priority.
...
* `target_group_arn` - (Optional, Editable) The Amazon Resource Name (ARN) of the target group to forward traffic.
...
* `message_body` - (Optional, Editable) The message of the response.
    * _Length of the value_: From 0 to 1024 symbols

#### forward

The `forward` block has the following structure:

* `target_group` - (Required, Editable) List of target groups to forward traffic.
  The structure of this block is [described below](#target_group).
    * _Size of the list_: From 2 to 5 elements

##### target_group

The `target_group` block has the following structure:

* `arn` - (Required, Editable) The Amazon Resource Name (ARN) of the target group to forward traffic.
    * _ARN Format_: `arn:c2:elasticloadbalancing::<project-name>@<customer-name>:targetgroup/tg-12345678`
...
* `host` - (Optional, Editable) The name of the host. This component is not percent-encoded.
    ...
    * _Length of the value_: From 3 to 255 symbols
    * _Constraints_: See all the constraints in [ELB API documentation][redirect-action-config]
* `path` - (Optional, Editable) The case-sensitive absolute path.
  ...
  This component is not percent-encoded.
    ...
    * _Length of the value_: From 1 to 128 symbols
...
* `query` - (Optional, Editable) The query parameters.
  ...
  This component is not percent-encoded.
    ...
    * _Length of the value_: From 0 to 128 symbols

### condition

The `condition` block has the following structure:

* `host_header` - (Optional, Editable) The block with information about the host headers the host name in the URL should match.
...
* `values` - (Required, Editable) List of the domain host names that the host name in the URL will be compared to.
  If multiple names are specified, a logical OR operator is applied for the condition.
    * _Length of the value_: From 3 to 255 symbols
    * _Constraints_: See all the constraints in [ELB API documentation][host-header-config]
...
* `values` - (Required, Editable) List of the path patterns that the path in the URL will be compared to.
  If multiple values are specified, a logical OR operator is applied for condition.
    * _Length of the value_: From 1 to 128 symbols
    * _Constraints_: See all the constraints in [ELB API documentation][path-pattern-config]
...
Listener rule can be imported using `arn`, e.g.,
