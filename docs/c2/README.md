...
Terraform registry может быть организован в виде s3 бакета с настроенным доступом по https.
...
|-- c2devel/                                       # разработчик
     |-- croccloud/                                # имя провайдера
          ...
          |         |-- linux/
          |         |    |-- amd64/
          |         |    |    |-- index.json       # метаинформация для сборки v1.0.0-CROC0_linux_amd64
          |         |    |-- ...
          |         |-- ...
          |    
          |-- versions/         
...
4. **Опционально.** Если был выполнен шаг 4: обновление ссылок в метаинформации
...
1. github actions: прогон тестов, прогон линтеров
2. testacc: выбор тестов, настройка тестового провайдера на C2 API + sweep + определение списка тестов
3. generate target: для чего используется
4. прогон линтеров: требуется ли export GOPATH / GOBIN. Возможно добавить в Makefile + provider lint.
   semgrep, provider-lint - находят замечания. make website-link-check - не отрабатывает
5. Определить набор архитектур и операционных систем, под которые собирается провайдер. Сейчас - совпадает с aws.
6. Использовать в make build команду go build вместо go install для того, чтобы иметь возможность задать имя артефакта
7. Поправить сообщение в PULL_REQUEST_TEMPLATE.md
8. Решить: обновлять ли go.mod и импорты
9. Релиз aws-sdk-go до провайдера
